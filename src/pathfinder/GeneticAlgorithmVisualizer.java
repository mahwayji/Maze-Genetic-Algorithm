...
public class GeneticAlgorithmVisualizer implements AnimatablePathfinder {
    ...
    public GeneticAlgorithmVisualizer(Maze maze) {
        ...
        this.MAX_STAGNANT = Math.max(maze.width(), maze.height()) * 5;
    ...
    public void nextStep() {
        ...
        if (bestMoves.goalReached || stagnantCount >= MAX_STAGNANT) {
            finished = true;
    ...
    private List<Integer> getValidMoves(Maze maze, Point pos, int prevMove) {
        ...
        if ((available & 8) != 0 && prevMove != 1)
            moves.add(0);
        if ((available & 4) != 0 && prevMove != 0)
            moves.add(1);
        if ((available & 2) != 0 && prevMove != 3)
            moves.add(2);
        if ((available & 1) != 0 && prevMove != 2)
            moves.add(3);
    ...
    private int biasedStep(int prev) {
        ...
        switch (prev) {
            case 0:
                forward = new int[] { 0, 3, 2 };
            case 1:
                forward = new int[] { 1, 3, 2 };
            case 2:
                forward = new int[] { 2, 1, 0 };
            case 3:
                forward = new int[] { 3, 1, 0 };
            default:
